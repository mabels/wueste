...
func getConfig() *eg.GeneratorConfig {
	return &eg.GeneratorConfig{
		...
		EntityCfg: eg.Config{
			...
			FromWueste:  "github.com/mabels/wueste/entity-generator/wueste",
			FromResult:  "github.com/mabels/wueste/entity-generator/rusty",
		},
	}
}
...
func TestGolang(t *testing.T) {
	...
	// GoGenerator(cfg, tfs, sl)
	// GoGenerator(cfg, eg.TestSchema(sl), sl)
	// for _, p := range sl.SchemaRegistry().Items() {
	// 	if !p.Written() {
	// 		continue
	// 	}
	// 	GoGenerator(cfg, p.PropertItem().Property(), sl)
	// }
}
