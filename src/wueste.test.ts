...
describe('string coerce', () => {
    ...
    it('string default', () => {
        const coerce = wuesten.AttributeString({
            jsonname: "x", varname: "x", default: "x", base: "base"
        })
...
describe('datetime coerce', () => {
    ...
    it('datetime default', () => {
        const coerce = wuesten.AttributeDateTime({
            jsonname: "x", varname: "x", base: "base",
            default: "2023-01-01"
        })
        expect(coerce.Get().unwrap()).toEqual(new Date("2023-01-01"))
    });
    ...
    it('datetimeoptional default', () => {
        const coerce = wuesten.AttributeDateTimeOptional({
            jsonname: "x", varname: "x",
            default: new Date("2023-01-01"), base: "base"
        })
        expect(coerce.Get().unwrap()).toEqual(new Date("2023-01-01"))
...
class Builder implements WuestenAttribute<Entity> {
    ...
    constructor(readonly param: WuestenAttributeParameter<Entity> = {
        jsonname: "builder",
        varname: "Builder",
        base: "base"
    }) {
    ...
    Coerce(value: unknown): Result<Entity, Error> {
        ...
        const errors = Object.values(results).filter(r => r.is_err()).map(r => r.unwrap_err().message)
        if (errors.length > 0) {
...
describe('object coerce', () => {
    ...
    it('object no default', () => {
        ...
        expect(coerce.Get().is_err()).toBeTruthy()
        expect(coerce.Coerce({
            id: "test",
        } as Entity).unwrap_err().message).toContain("Attribute[super.x.test] not found")
        expect(coerce.Coerce({
            id: "test",
            Test: 6.4
        } as unknown as Entity).is_ok()).toBeTruthy()
        ...
        expect(coerce.CoerceAttribute({ "X": { id: { toString: 4 }, Test: "bla" } }).unwrap_err().message).toContain("Attribute[super.x.id] is not a string")
        expect(coerce.CoerceAttribute({ "x": { id: { toString: 4 }, Test: "bla" } }).unwrap_err().message).toContain("Attribute[super.x.test] is not a number")
